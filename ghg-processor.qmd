...
params:
  ...
  data_folder: "data-peixoto/"
  output_folder: "output/"
  metadata_folder: "data-peixoto/"
  MSMT_LENGTH_SEC: 300
  DEAD_BAND_SEC: 10
...
read_ghg_data <- function(fn) {
  ...
  dat <- try({
    readr::read_table(I(dat_raw), na = "nan", guess_max = 1e5)
  })
...
#  message("Metadata row ", i, " ", metadat$Plot[i], " start = ", start_time, " matched ", nrow(x), " data rows")
  ...
  if(nrow(x)) {
    ...
    zero_matches <- rbind(zero_matches, 
                          data.frame(File = metadat$Metadata_file[i],
                                     Row = i,
                                     Date = metadat$Date[i],
                                     Time_start = format(metadat$Time_start[i], format = "%H:%M:%S")))
  }
...
if(n == 0) { # no matched data at all
  cat("**<span style='color:red'>---> No data were matched at all! <---</span>**\n\n")
  knitr::knit_exit()
}
```

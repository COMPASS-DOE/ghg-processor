...
```{r}
# General function: plot distribution of a column,
# identify outliers
plot_and_flag <- function(results, col, combined_dat,
                          probs = c(0.05, 0.95),
                          flag_name = paste0("FLAG_", toupper(col))) {
  
  ggplot(results, aes(.data[[col]])) + 
    geom_histogram(bins = 30) +
    facet_wrap(Gas ~., scales = "free", ncol = 1)
  
  # Find which ones are outside of quantiles
  # Print the observation numbers
  # Assign a flag
  # Graph them
  x_split <- split(x[,col], x$Gas)
  quants <- lapply(x_split, quantile, probs = probs)
}
```
...
#| fig-width: 8
#| eval: false
plot_and_flag(results, "int_estimate", combined_dat)
```
...
twocol_plot_and_flag <- function(results, col1, col2,
  ...
  p <- p +
    ...
    geom_label_repel(data = subset(results, resid_flags), 
                     aes(label = paste(paste0("#", Obs), Date, "\n", Plot, "\nsresid =", rstandard)), 
                     size = 2.5) +
    facet_wrap(Gas ~., scales = "free", ncol = 1)
  print(p)
...
plot_group <- function(results, combined_dat) {
  ...
  plotdata_long <- tidyr::pivot_longer(plotdata, c("CO2", "CH4"),
                                      names_to = "Gas")
  post_deadband <- subset(plotdata_long, ELAPSED_SECS > DEAD_BAND_SEC)
  ...
  ggplot(plotdata_long, aes(ELAPSED_SECS, value, group = Obs)) + 
    ...
    geom_smooth(data = post_deadband,
...
Two column tests
  Polynomial R2 versus linear R2
  Robust slope versus linear slope
  
  In these cases we want to graph relationship
  Identify outliers
    Cook's distance not appropriate; we don't care about leverage here
    Use rstandard (standardized residuals)
  Plot x versus y, coloring by rstandard > 2 or not? And label those?
  Then for those points, plot raw data
...
R2
Flux difference or R2 difference between lm and robust
Flux difference or R2 difference between lm and poly

Do we look for 5%/95% values?
Absolute threshold?
MAD?
...
## Observations with custom dead band

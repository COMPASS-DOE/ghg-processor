...
for(i in seq_len(nrow(metadat))) {
  ...
  if(nrow(x)) {
    ...
    # Assign stop time (in seconds, not including dead band): value given in Quarto params, but can be overridden in metadata    
...
plot_group <- function(results, combined_dat) {
  ...
  plotdata_long <- tidyr::pivot_longer(plotdata, c("CO2", "CH4"),
                                       names_to = "Gas")
  model_data <- subset(plotdata_long, ELAPSED_SECS > DEAD_BAND_SEC & ELAPSED_SECS <= MSMT_STOP_SEC)
...
all_dat_low_r2 <- subset(combined_dat, Obs %in% low_r2,
                         select = c("Obs", "Plot", "TIMESTAMP", "ELAPSED_SECS",
                                    "DEAD_BAND_SEC", "MSMT_STOP_SEC", "CO2", "CH4"))
